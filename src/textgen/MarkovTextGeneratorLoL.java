...
public class MarkovTextGeneratorLoL implements MarkovTextGenerator {
	...
	public void train(String sourceText)
	{
		...
		for(int i = 0; i < wordArray.length-1; i++) {	
			ListNode node = getNode(wordArray[i]);
				node.addNextWord(wordArray[i+1]);
		}
		wordList.add(new ListNode(wordArray[wordArray.length - 1]));
		ListNode node = wordList.get(wordList.size() -1);
		node.addNextWord(wordArray[0]);
	}
	...
	public String generateText(int numWords) {
		...
		for(int i = 0; i < numWords - 1; i++) {
			ListNode temp = getNode(currWord);
